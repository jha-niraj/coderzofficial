...
function SignInForm({ searchParams }: SignInFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // Validate credentials first to get specific error messages
            ...
            if (result?.ok) {
                ...
                // Check if this is a Learn platform SSO request
                const isLearnPlatformRequest = callbackUrl?.includes('learn.') ||
                    searchParams?.get('redirect_uri')?.includes('learn.');

                if (isLearnPlatformRequest) {
                    // Redirect to our SSO endpoint to generate JWT and redirect to Learn platform
                    const redirectUri = searchParams?.get('redirect_uri');
                    const state = searchParams?.get('state');
            ...
            >
                ...
                <div className="hidden md:flex absolute z-[2] overflow-hidden backdrop-blur-2xl pointer-events-none">
                    {[...Array(6)].map((_, i) => (
                        <div 
                            key={i}
                            className="h-[40rem] z-[2] w-[4rem] bg-gradient-to-r from-transparent via-black/20 to-white/10 opacity-30"
                        />
                    ))}
                </div>
                ...
                <div className="p-8 md:p-12 md:w-1/2 flex flex-col bg-white dark:bg-neutral-900 z-10">
                    ...
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
                        ...
                        <Button
                            ...
                            className="w-full border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-800"
                        >
                            {googleSignIn ? (
                                <>
                                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
