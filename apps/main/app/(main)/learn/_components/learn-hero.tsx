...
interface LearnsHeroProps {
    totalLearns?: number;
    totalSteps?: number;
    totalCategories?: number;
}

export default function LearnsHero({ totalLearns = 0, totalSteps = 0, totalCategories = 0 }: LearnsHeroProps) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (search.trim()) {
            router.push(`/Learns/browse?search=${encodeURIComponent(search.trim())}`);
        }
    };
                ...
                >
                    ...
                    >
                        <div className="text-center">
                            <div className="text-3xl font-bold text-neutral-900 dark:text-white">{totalLearns}</div>
                            <div className="text-sm text-neutral-500 dark:text-neutral-400">Learns</div>
                        </div>
