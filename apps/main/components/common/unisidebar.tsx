...
import { uniNavigation, NavigationItem } from "@/lib/uni-navigation"
...
function UniSidebarContent({ universityName, credits }: UniSidebarProps) {
    ...
    const isActive = (path: string) => {
        if (path === "uni" && pathname === "/uni") return true
    ...
    const renderNavItem = (item: NavigationItem, _isSecondary = false) => {
                        ...
                        <TooltipTrigger asChild>
                            <Link
                                ...
                                className={cn(
                                    ...
                                    active && "bg-violet-50 dark:bg-violet-900/20 text-violet-600 dark:text-violet-400",
                                    item.comingSoon && "opacity-50 pointer-events-none"
                                )}
                            >
                                <Icon className={cn(
                                    "w-5 h-5",
                                    active ? "text-violet-600 dark:text-violet-400" : "text-neutral-600 dark:text-neutral-400"
                                )} />
                            </Link>
                        </TooltipTrigger>
        ...
        return (
            ...
            >
                ...
                <span className={cn(
                    "text-sm font-medium",
                    active ? "text-violet-600 dark:text-violet-400" : "text-neutral-700 dark:text-neutral-300"
                )}>
                    {item.name}
                </span>
                {item.comingSoon && (
                ...
                {item.badge && (
                    <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 font-medium">
                        {item.badge}
                    </span>
                )}
            </Link>
    ...
    return (
        ...
        >
                ...
                {!isCollapsed && (
                    <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                            <span className="font-bold text-neutral-900 dark:text-white text-sm block truncate">
                                {universityName || "University"}
                            </span>
                            <span className="text-[10px] text-neutral-500">Student Portal</span>
                        </div>
                    </div>
                )}
                {isCollapsed && (
            ...
            {!isCollapsed && (
                <div className="p-3 border-b border-neutral-200 dark:border-neutral-800">
                    <div className="bg-gradient-to-r from-violet-500/10 to-indigo-500/10 dark:from-violet-500/20 dark:to-indigo-500/20 rounded-xl p-3">
                        <div className="flex items-center gap-2">
                            <Coins className="w-4 h-4 text-violet-600 dark:text-violet-400" />
                            <span className="text-xs text-neutral-600 dark:text-neutral-400">University Credits</span>
                        </div>
            ...
            <ScrollArea className="flex-1 px-3 py-4">
                ...
                <div className="space-y-1 mb-6">
                    ...
                    {uniNavigation.primary.map(item => renderNavItem(item))}
                </div>
                ...
                <div className="space-y-1">
                    ...
                    {uniNavigation.secondary.map(item => renderNavItem(item, true))}
                </div>
            </ScrollArea>
...
export default function UniSidebar({ universityName, credits }: UniSidebarProps) {
    return <UniSidebarContent universityName={universityName} credits={credits} />
}
