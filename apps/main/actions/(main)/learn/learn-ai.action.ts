...
- Analogies and real-world examples`;
                break;
            case "CODE":
                typeSpecificInstructions = `Write content introducing code Learns in **Markdown format**. Include:
- A brief explanation of the Learn
...
- Use \`\`\`language code blocks for examples`;
            ...
            case "CHALLENGE":
                ...
                break;
            case "SUMMARY":
                typeSpecificInstructions = "Write a recap of the key Learns in **Markdown format**. Be concise but comprehensive.";
                break;
            case "RESOURCE":
...
export async function generateQuizQuestions(
    ...
    language?: string
): Promise<{ questions?: GeneratedQuiz[]; error?: string }> {
    try {
        const adminCheck = await checkIsAdmin();
        if (!adminCheck.isAdmin) {
            return { error: adminCheck.error };
...
Generate ${count} multiple-choice quiz questions that test understanding of this step.
Requirements for EACH question:
1. Clear and unambiguous question
2. Exactly 4 options with unique string IDs (a, b, c, d)
3. Only one correct answer per question
4. Detailed explanation in Markdown format
5. Questions should cover different aspects of the topic
6. Mix difficulty levels within the set
...
Respond in JSON format:
{
    "questions": [
        ...
        const result = JSON.parse(cleanContent) as { questions: GeneratedQuiz[] };
...
export async function generateResources(
    ...
    try {
        ...
        const query = `Find the best YouTube tutorials and official documentation for learning "${stepTitle}" in the context of ${LearnTitle}. Category: ${category}, difficulty level: ${difficulty}. Include high-quality, up-to-date resources.`;
...
export async function askLearnAssistant(
    LearnTitle: string,
    LearnDescription: string,
    ...
    try {
        ...
        const messages: { role: "system" | "user" | "assistant"; content: string }[] = [
            {
                role: "system",
                content: `You are an expert programming tutor helping a student learn about "${LearnTitle}".
...
- Relate answers back to the current Learn and step
...
export interface GeneratedProject {
    ...
    difficulty: string;
    requirements: string; // Markdown formatted
    learningObjectives: string[];
}

export async function generateLearnProjects(
    LearnTitle: string,
    LearnDescription: string,
    projectCount: number = 3,
    projectType: "minor" | "major" = "minor",
    technologies?: string[],
    difficulty?: string
): Promise<{ projects?: GeneratedProject[]; error?: string }> {
    try {
        const adminCheck = await checkIsAdmin();
        if (!adminCheck.isAdmin) {
            return { error: adminCheck.error };
        ...
        const count = Math.min(Math.max(projectCount, 1), 6);
        const techContext = technologies && technologies.length > 0
            ? `Preferred Technologies: ${technologies.join(", ")}`
            : "";

        const typeInstructions = projectType === "minor"
            ? "Each project should be a focused, small-scale project that can be completed in 1-3 hours. Think of exercises that reinforce the Learn through building."
            : "Each project should be a comprehensive, portfolio-worthy project that takes 5-15 hours. Include multiple features, best practices, and real-world scenarios.";

        const prompt = `You are an expert programming instructor generating ${count} ${projectType} project ideas for students learning a Learn.
...
3. Technologies required (3-6 technologies)
4. Estimated hours to complete
5. Difficulty level (BEGINNER, INTERMEDIATE, or ADVANCED)
6. Detailed requirements in Markdown format (include headers, bullet points, code examples where applicable)
7. 3-5 learning objectives

The requirements should be comprehensive and in Markdown format with:
- **## Project Overview** section
- **## Requirements** section with numbered list  
- **## Bonus Features** section (optional extras)
- Code snippets or API examples where relevant
    ...
    "projects": [
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a helpful assistant that generates project ideas for programming education. Always respond with valid JSON only."
                },
                { role: "user", content: prompt }
            ],
            temperature: 0.8,
        ...
        const content = response.choices[0]?.message?.content;
        if (!content) {
            return { error: "Failed to generate projects" };
        }

        // Clean content of markdown code blocks if present
        const cleanContent = content.replace(/```json\n?|\n?```/g, "").trim();

        const result = JSON.parse(cleanContent) as { projects: GeneratedProject[] };
        return { projects: result.projects };
    } catch (error) {
        console.error("Error generating Learn projects:", error);
        return { error: "Failed to generate projects" };
    }
}