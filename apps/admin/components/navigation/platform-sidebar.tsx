...
import {
    ...
    ChevronDown, AlignLeft, type LucideIcon
} from "lucide-react";
import {
...
export function PlatformSidebar({ platform }: PlatformSidebarProps) {
    ...
    }, [pathname, platform.navItems]);

    const toggleExpanded = (path: string) => {
        setExpandedItems(prev =>
            prev.includes(path) ? prev.filter(p => p !== path) : [path]
        );
    };
    ...
    const renderNavItem = (item: NavItem, depth = 0) => {
        ...
        const isExpanded = expandedItems.includes(item.path) || (isChildActive && !isCollapsed);
                ...
                <div key={item.path} className="space-y-1">
                    <button
                        onClick={() => {
                            ...
                            if (!expandedItems.includes(item.path)) setExpandedItems([item.path]);
                        }}
                        className={cn(
                            "cursor-pointer flex items-center w-full gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all",
                    ...
                    >
                        ...
                        {!isCollapsed && (
                            <>
                                ...
                                <div
                                    onClick={(e) => { e.stopPropagation(); toggleExpanded(item.path); }}
    ...
    const sidebarContent = (
        <>
            {/* Header */}
            <div className={cn(
                "p-6 flex items-center relative border-b border-neutral-200 dark:border-neutral-800",
                isCollapsed ? "justify-center" : "gap-3"
            )}>
                <Link href={platform.overviewHref} className="flex items-center gap-3">
                    <div className={cn("w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0", platform.bgColor)}>
                        <PlatformIcon className="w-5 h-5 text-white" />
                    </div>
                    {!isCollapsed && (
                        <div className="flex-1 text-left min-w-0 hidden lg:block">
                            <h1 className="font-bold text-neutral-900 dark:text-white truncate tracking-tight">{platform.name}</h1>
            ...
            <ScrollArea className="flex-1">
                <nav className="px-3 py-4 space-y-1">
                    ...
                    <Link
                        href="/dashboard"
                        className={cn(
                            "flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm font-medium transition-all mb-3",
                    ...
                    <div className="border-t border-neutral-200 dark:border-neutral-800 pt-3 space-y-1">
                        {!isCollapsed && (
                            <div className="flex items-center gap-2 px-2 mb-2">
                                <div className={cn("w-1 h-3 rounded-full", platform.color)} />
                                <p className={cn("text-[10px] font-mono font-bold uppercase tracking-widest", platform.textColor)}>
                                    {platform.name}
                                </p>
                            </div>
                        )}
                        {platform.navItems.map(item => renderNavItem(item))}
                    </div>
                </nav>
            </ScrollArea>
            ...
            <div className="mt-auto border-t border-neutral-200 dark:border-neutral-800">
                <div className={cn("p-2", isCollapsed ? "flex justify-center" : "grid grid-cols-2 gap-2")}>
                    ...
                    {session && (
                        <div className="relative" onMouseEnter={handleNotificationsMouseEnter} onMouseLeave={handleNotificationsMouseLeave}>
                            <button className={cn(
                                "flex items-center justify-center rounded-lg p-2 text-sm font-medium transition-all hover:bg-neutral-100 dark:hover:bg-neutral-800 w-full",
                                isCollapsed && "aspect-square"
                            )}>
                                <div className="relative">
                                    ...
                                    {unreadCount > 0 && (
                                        <span className="absolute -top-1 -right-1 h-3 w-3 rounded-full bg-red-600 border border-white dark:border-neutral-950" />
                                    )}
                                </div>
                                    ...
                                    <div className="max-h-80 overflow-y-auto">
                                        {notifications.length === 0 ? (
                                            ...
                                            notifications.map(n => (
                                                <button
                                                    key={n.id}
                                                    onClick={() => { if (n.actionUrl) router.push(n.actionUrl); setNotificationsDropdownOpen(false); }}
                                                    className={cn(
                                                ...
                                                >
                                                    ...
                                                    <p className="text-xs text-neutral-500 mt-1">{format(new Date(n.createdAt), "MMM d, h:mm a")}</p>
                                                </button>
                                            ))
                                        )}
                                    </div>
                ...
                {session ? (
                    <div className="relative px-3 py-2" onMouseEnter={handleProfileMouseEnter} onMouseLeave={handleProfileMouseLeave}>
                        <button className={cn(
                            "flex cursor-pointer items-center gap-3 w-full rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 p-2 transition-colors",
                            ...
                            >
                                <div className="p-2">
                                    <button
                                        ...
                                        className="cursor-pointer w-full flex items-center gap-3 px-3 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-md transition-colors text-sm"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="cursor-pointer w-full flex items-center gap-3 px-3 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 rounded-md transition-colors text-sm"
                                    >
                    ...
                    <div className="px-3 py-2">
                        <button
                            onClick={() => router.push("/")}
                            className={cn("flex items-center w-full rounded-lg p-2 text-sm font-medium hover:bg-neutral-100 dark:hover:bg-neutral-800", isCollapsed && "justify-center")}
                        >
