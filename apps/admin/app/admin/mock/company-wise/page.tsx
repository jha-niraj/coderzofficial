...
export default function CompanyMockAdminPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const [sessionsRes, statsRes] = await Promise.all([
                getAdminMockSessions(1, 10, 'COMPANY'),
                getAdminMockStats()
            ]);
            setData({
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-foreground">Company Mock Sessions</h1>
                </div>
            </div>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Total Company Mocks</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{data.stats?.totalCompany ?? 0}</div>
                    </CardContent>
                </Card>
            </div>
            <Card>
                <CardHeader>
                    <CardTitle>Company Mock Sessions</CardTitle>
                </CardHeader>
                <CardContent>
                    {
                        data.sessions.length > 0 ? (
                            ...
                            <EmptyState icon={Briefcase} title="No Company Mock Sessions" description="No company mock sessions found." />
                        )
                    }
                </CardContent>
            </Card>
